    ...
    <div className="fixed bottom-6 right-6 z-[9999] font-noto">
      ...
      <div className="relative flex flex-col items-end gap-3">
        ...
        {isOpen && hoveredRole !== null && (
          ...
          >
            <div className="px-3 py-2 border-b border-slate-100 mb-3">
              <p className="text-[10px] font-black text-[#0866FF] uppercase tracking-[0.2em] mb-1">Personnel Selection</p>
            ...
            <div className="space-y-1.5 max-h-[320px] overflow-y-auto no-scrollbar">
              {MOCK_USERS.filter(u => u.role === hoveredRole).map((u) => (
                <button
                  ...
                  className={cn(
                    "w-full flex flex-col items-start px-4 py-2.5 rounded-2xl transition-all duration-200 hover:bg-[#0866FF]/5 group",
                    user.id === u.id ? "bg-[#0866FF]/10 ring-1 ring-[#0866FF]/20" : ""
                  )}
                >
                  <span className={cn("text-[13px] font-bold", user.id === u.id ? "text-[#0866FF]" : "text-slate-700")}>
                    {u.name}
                  </span>
          ...
          <div className="bg-white/90 backdrop-blur-xl border border-slate-200 rounded-[2.5rem] p-4 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300 min-w-[220px]">
            <div className="px-4 py-3 border-b border-slate-100 mb-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-medium">Viewing as: <span className="text-[#0866FF]">{user.name}</span></p>
              </div>
              ...
              {roleCategories.map((r) => (
                <div
                  ...
                  className={cn(
                    ...
                    hoveredRole === r.id ? "bg-[#0866FF]/5" : "hover:bg-slate-50"
                  )}
                >
                  ...
                  <ChevronRight size={14} className={cn("text-slate-300 transition-all", hoveredRole === r.id ? "translate-x-1 text-[#0866FF]" : "")} />
                </div>
              ))}
            </div>
        ...
        <button
          ...
          className={cn(
            "w-15 h-15 rounded-full flex items-center justify-center shadow-3xl transition-all duration-500 hover:scale-105 active:scale-95 group relative overflow-hidden ring-4 ring-white",
            isOpen ? "bg-slate-900 rotate-90" : "bg-[#0866FF]"
          )}
        >
        ...
        <div className="bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full border border-slate-100 shadow-xl pointer-events-none select-none flex items-center gap-3">
          <div className={cn(
            "w-2 h-2 rounded-full",
            user.role === Role.Developer ? "bg-blue-500 animate-pulse" : "bg-emerald-500"
          )} />
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
            {user.role === Role.Developer ? "System Root" : `User: ${user.name}`}
          </p>
        </div>
      </div>
    </div>
